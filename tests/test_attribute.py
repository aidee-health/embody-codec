...
class TestAttributes:
    ...
    def test_serial_no_attribute_format_value(self):
        assert (
            attributes.SerialNoAttribute(12345678).formatted_value()
            == "0000000000bc614e"
        )
    ...
    def test_firmware_version_format_value(self):
        assert (
            attributes.FirmwareVersionAttribute(0x010203).formatted_value()
            == "01.02.03"
        )
    ...
    def test_encode_decode_no_of_ppg_values(self):
    ...
    def test_convert_pulse_raw_list_format_from_complex_byte(self):
        fmt, ecg_length, ppg_length = types.PulseRawList.to_format_and_lengths(0x37)
        assert fmt == 3
        assert ecg_length == 1
        assert ppg_length == 3

    def test_convert_pulse_raw_list_format_to_complex_byte(self):
    ...
    def test_encode_decode_breath_rate(self):
        do_test_encode_decode_attribute(
            attributes.BreathRateAttribute(value=16), b"\x10"
        )
    ...
    def test_encode_decode_charge_state(self):
        decoded = do_test_encode_decode_attribute(
            attributes.ChargeStateAttribute(value=1), b"\x01"
        )
        assert decoded.value is True

    def test_encode_decode_belt_on_body_state(self):
        decoded = do_test_encode_decode_attribute(
            attributes.BeltOnBodyStateAttribute(value=0), b"\x00"
        )
    ...
    def test_encode_decode_pulse_raw_attribute(self):
        do_test_encode_decode_attribute(
            attributes.PulseRawAttribute(value=types.PulseRaw(ecg=1, ppg=1000)),
            b"\x00\x00\x00\x01\x00\x00\x03\xe8",
        )

    def test_encode_decode_pulse_raw_all_attribute(self):
        do_test_encode_decode_attribute(
            attributes.PulseRawAllAttribute(
                value=types.PulseRawAll(ecg=1, ppg_green=1000, ppg_red=100, ppg_ir=5)
            ),
